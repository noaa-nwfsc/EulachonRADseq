...
#load vcf file
setwd("~/Desktop/eulachon/eulachon_WR/NWFSC_RAD/stacks_RADseq_MAF05r65p9_indvremoved")
vcfData<-read.vcfR("stacks_RADseq_MAF05r65p9_indvremoved_populations.snps.vcf")
...
ggplot(IDdupResults, aes(proportionCommon,proportionMatch, 
                         label = Sample1)) + 
  geom_point(alpha=0.1) +
  geom_text_repel(data = subset(IDdupResults, proportionMatch > 0.9)) 
ggsave("eulachon_IDdups.png")
# Klamath 51985.7 and .8 are the same sample
...
save.image("eulachon_IDdups.RData")
